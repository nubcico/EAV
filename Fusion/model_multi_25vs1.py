...
class TrainerMultiModal:
    ...
    def _prepare_dataloader(self, x_audio, y_audio, x_video, y_video, shuffle=False):
        ...
        x_audio = x_audio.repeat(len(x_video) // len(x_audio), 1, 1, 1)  # Repeat audio data to match video batches
    ...
    def train(self, freeze, epochs=20, lr=None):
        ...
        for epoch in range(epochs):
            ...
            with open('aud_video_results_new.txt', 'a') as f:
                f.write(f"Subject {self.sub} Epoch {epoch + 1} Training Accuracy: {training_accuracy:.4f}\n")
