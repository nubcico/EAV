...
class MultiModalViT(nn.Module):
    ...
    def forward(self, audio_x, video_x):
        ...
        video_features = self.video_model.feature(video_x)[:, 0]  # Extract only class token features
...
class TrainerMultiModal:
    def __init__(self, multimodal_model, data_audio, data_video, lr=1e-4, batch_size=32, num_epochs=10, sub=''):
        ...
        self.train_dataloader = self._prepare_dataloader(data_audio[0], data_audio[1], data_video[0], data_video[1],
                                                         shuffle=True)
        self.test_dataloader = self._prepare_dataloader(data_audio[2], data_audio[3], data_video[2], data_video[3],
                                                        shuffle=False)
    ...
    def _prepare_dataloader(self, x_audio, y_audio, x_video, y_video, shuffle=False):
        # Ensure audio and video data are tensors
        
        x_video = self.preprocess_images(x_video).view(-1, 3, 224, 224)
        y_video = torch.from_numpy(np.repeat(y_video, 25)).long()
        
        x_audio = x_audio.repeat_interleave(25, dim=0)
        x_audio = torch.tensor(x_audio, dtype=torch.float32)
        y_audio = torch.from_numpy(np.repeat(y_audio, 25)).long()
        ...
        for epoch in range(epochs):
            ...
            for i, (audio_data, video_data) in enumerate(self.train_dataloader):
                ...
                with autocast():  # Mixed precision training
                    ...
                    # video_output = self.multimodal_model.video_model(video_inputs)

                    # # Calculate additional losses
                    # audio_loss = self.criterion(audio_output, audio_labels)
                    # video_loss = self.criterion(video_output, video_labels)
                    
                    audio_loss = 0
                    video_loss = 0
                    
                    # # Delete intermediate outputs to save memory
                    # del audio_output
                    # del video_output
                    torch.cuda.empty_cache()
                    ...
                    #torch.nn.utils.clip_grad_norm_(self.multimodal_model.video_model.parameters(), max_norm=1.0)
                    
                    self.optimizer.step()
                    
                    _, predicted = torch.max(scores, 1)
                    total_correct += (predicted == audio_labels).sum().item()
            ...
            with open('aud_video_results_new.txt', 'a') as f:
                f.write(f"Subject {self.sub} Epoch {epoch + 1} freeze {freeze} Training Accuracy: {training_accuracy:.4f}\n")
                f.write(f"Subject {self.sub} Epoch {epoch + 1} freeze {freeze} Accuracy: {accuracy}\n")
...
for i in str:
    ...
    trainer = TrainerMultiModal(multimodal_model = combined_model, data_audio = data_AUD,
                                data_video = data_video, batch_size=8, num_epochs = 30, sub = sub)

    trainer.train(freeze=True, epochs=10, lr=5e-4)
