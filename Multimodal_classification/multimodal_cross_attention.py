...
class CrossAttention(nn.Module):
    def __init__(
        ...
        self.query = nn.Linear(dim, self.all_head_size, bias=qkv_bias)
        self.key = nn.Linear(dim, self.all_head_size, bias=qkv_bias)
        self.value = nn.Linear(dim, self.all_head_size, bias=qkv_bias)
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, output_attentions: bool = False) -> Union[Tuple[torch.Tensor, torch.Tensor], torch.Tensor]:
        B, N, C = x1.shape
        #print(x)
        query = self.transpose_for_scores(self.query(x1)) # B x num_heads x N x head_dim
        key = self.transpose_for_scores(self.key(x2))  # B x num_heads x N x head_dim
        value = self.transpose_for_scores(self.value(x2))  # B x num_heads x N x head_dim
...
class MultiModalViT(nn.Module):
    ...
    def forward(self, audio_x, video_x):
       ...
       #print(video_features.shape)
        ...
        audio_features = audio_features + self.cross_attn(audio_features, video_features)
        video_features = video_features + self.cross_attn(video_features, audio_features)
        ...
        combined_features = torch.cat((audio_features[:,0], video_features[:,0]), dim=1)
...
class TrainerMultiModal:
    ...
    def train(self, freeze, epochs=20, lr=None):
        ...
        for epoch in range(epochs):
            ...
            with open('aud_video_results_cross_attention.txt', 'a') as f:
...
for i in range(1,43):
    ...
    model_path = f"D:\.spyder-py3\Finetuned_models_ratio7030\model_with_weights_audio_finetuned_{i}.pth"
    ...
    model_path = f"D:\.spyder-py3\Finetuned_models_ratio7030\model_with_weights_video_finetuned_{i}.pth"
    model_vid.load_state_dict(torch.load(model_path))
    
    combined_model = MultiModalViT(model_aud, model_vid)

    direct=r"C:\Users\user.DESKTOP-HI4HHBR\Downloads\Audio(1)\Audio"
    file_name = f"subject_{i:02d}_aud.pkl"
    ...
    file_ = os.path.join(r"C:\Users\user.DESKTOP-HI4HHBR\Downloads\Vision\Vision", file_name)
    ...
    trainer.train(freeze=True, epochs=15, lr=5e-4)#best epoch 9th
